...
RSpec.describe Puppet::Provider::CheckHttp::CheckHttp do
  ...
  let(:valid_hash) do
    { name: 'foo', url: valid_uri, ensure: 'present', expected_statuses: [200], body_matcher: %r{Google}, request_timeout: 30, retries: 3, backoff: 1, exponential_backoff_base: 2, max_backoff: 40,
   timeout: 60 }
  end
  let(:invalid_hash) do
    { name: 'foos', url: invalid_uri, ensure: 'present', expected_statuses: [200], body_matcher: %r{Google}, request_timeout: 30, retries: 3, backoff: 1, exponential_backoff_base: 2, max_backoff: 40,
   timeout: 60 }
  end
  ...
  describe 'insync?(context, name, attribute_name, is_hash, should_hash) expected_status not matching' do
    it 'processes resources' do
      ...
      allow(context).to receive(:debug)
      expect(context).to receive(:debug).with('Checking whether foo is up-to-date')
      expect { provider.insync?(context, 'foo', 'foo', invalid_hash, invalid_hash) }.to raise_error(%r{check_http response code check failed.})
    end
  end
  ...
  describe 'insync?(context, name, attribute_name, is_hash, should_hash) body_matcher not matching' do
    it 'processes resources' do
      ...
      expect(context).to receive(:debug).with('Checking whether foo is up-to-date')
      expect(context).to receive(:debug).with("The return response '200' is matching with the expected_statuses '[200]'")
      expect { provider.insync?(context, 'foo', 'foo', invalid_hash, invalid_hash) }.to raise_error(%r{check_http response body check failed.})
    end
  end
  ...
  describe 'insync?(context, name, attribute_name, is_hash, should_hash) with Retry' do
    it 'processes resources' do
      ...
      expect(context).to receive(:info).with(%r{StandardError: 'Exception from WebMock' - 1 tries})
      expect(context).to receive(:info).with(%r{StandardError: 'Exception from WebMock' - 2 tries})
      expect(context).to receive(:info).with(%r{StandardError: 'Exception from WebMock' - 3 tries})
